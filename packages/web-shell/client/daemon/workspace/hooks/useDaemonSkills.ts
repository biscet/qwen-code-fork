...
export function useDaemonSkills(options: DaemonSkillsOptions = {}) {
  ...
  const getDetail = useCallback(
    (skill: DaemonWorkspaceSkillIdentity) => {
      if (!splitRuntimeAvailable) {
        requireLegacyPrimary(legacyWorkspaceMismatch);
        return actions.loadSkillDetail(skill);
      }
      if (!workspaceClient) throw new Error('Workspace is unavailable.');
      return workspaceClient.workspaceSkillDetail(skill);
    },
    [actions, legacyWorkspaceMismatch, splitRuntimeAvailable, workspaceClient],
  );
  return {
    getDetail,
