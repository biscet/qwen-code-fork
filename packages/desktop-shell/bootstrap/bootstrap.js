...
async function chooseWorkspace() {
  ...
  setStatus(
    'starting',
    'Opening workspace',
    'Starting the bundled HomeCode runtime…',
  );
  try {
...
async function installUpdate() {
  ...
  try {
    ...
    setStatus(
      'error',
      'Update failed',
      'HomeCode remains usable. Try again or update manually.',
      String(failure),
    );
  } finally {
...
initialize().catch((failure) => {
  setStatus(
    ...
    'Restart HomeCode or inspect the desktop log.',
    String(failure),
  );
});
