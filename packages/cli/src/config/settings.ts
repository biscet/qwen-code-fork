...
export class LoadedSettings {
  ...
  private computeMergedSettings(): Settings {
    ...
    const research = merged.mcpServers?.['home-ai-research'];
    if (
      process.env['QWEN_CODE_DESKTOP'] !== '1' ||
      research?.httpUrl !== 'https://biscet-server.local:9454/research/mcp'
    ) {
      return merged;
    }
    const original = mergeSettings(
      this.system.originalSettings,
      this.systemDefaults.originalSettings,
      this.user.originalSettings,
      this.workspace.originalSettings,
      this.isTrusted,
    ).mcpServers?.['home-ai-research'];
    if (
    ...
    try {
      apiKey = new HomeChatStateStore().read().backendApiKey;
    } catch {
      debugLogger.warn('Unable to read the Vane MCP API key.');
      return merged;
    }
    if (apiKey) {
      merged.mcpServers = {
        ...
        'home-ai-research': {
          ...research,
          headers: { ...research.headers, Authorization: `Bearer ${apiKey}` },
        },
      };
    }
    return merged;
