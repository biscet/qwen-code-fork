...
export function ChatPane({
  ...
  const handleSubmit = useCallback(
    (
      ...
      if (!codexActive && !planOperation && /^\/goal(?:\s|$)/i.test(trimmed)) {
  ...
  const paneToolbarActions = useMemo(
    () =>
      codexActive
        ...
        : showWorkspaceChip
          ? [...PANE_TOOLBAR_ACTIONS, 'workspace' as const].filter(
              (action) => action !== 'plan' || planControlVisible,
            )
          : PANE_TOOLBAR_ACTIONS.filter(
              (action) => action !== 'plan' || planControlVisible,
            ),
    [showWorkspaceChip, codexActive, planControlVisible],
  );
  const headerActions =
