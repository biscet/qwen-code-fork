...
export function createServeApp(
  ...
  registerHomeChatRoutes(app, {
    ...
    onConnectionChanged: async () => {
      const { initializeDesktopWorkspaceDefaults } = await import(
        './desktop-workspace-defaults.js'
      );
      const runtimes = workspaceRegistry
        .listManaged()
        .filter(
          (runtime) => runtime.trusted && !runtime.generationGuard?.closed,
        );
      for (const runtime of runtimes) {
        if (runtime.generationGuard?.closed) continue;
        try {
          await initializeDesktopWorkspaceDefaults(runtime.workspaceCwd, true);
        } catch {
          writeStderrLine(
            'qwen serve: unable to refresh desktop MCP approvals',
          );
        }
      }
      for (const runtime of runtimes) {
